...
// CuoreUI, Unlicense, cuiColorPickerWheel.cs https://github.com/owtysm2/CuoreUI/blob/4dd8100ba84e4c9bedda003755c695737ab252d0/CuoreUI/Controls/cuiColorPickerWheel.cs#L15
...
namespace ThemeEngineTest.Controls
{
    ...
    public partial class Color_Picker_Wheel_Control : UserControl
    {
        private Bitmap privateHueBitmap;
        private Bitmap privateTriangleBitmap;

        public Color_Picker_Wheel_Control()
        {
            InitializeComponent();
        ...
        // CuoreUI, Unlicense, DrawingHelper.cs https://github.com/owtysm2/CuoreUI/blob/master/CuoreUI/Helpers/DrawingHelper.cs#L113
        ...
        // CuoreUI, Unlicense, GeneralHelper.cs https://github.com/owtysm2/CuoreUI/blob/master/CuoreUI/Helpers/GeneralHelper.cs#L436
        ...
        {
            var bestMatch = ClosestPointOnSegment(p, a, b);
            float bestD = DistanceSquared(p, bestMatch);

            var q = ClosestPointOnSegment(p, b, c);
            float d = DistanceSquared(p, q);
            if (d < bestD) { bestMatch = q; bestD = d; }

            q = ClosestPointOnSegment(p, c, a);
            if (DistanceSquared(p, q) < bestD) bestMatch = q;

            return bestMatch;
        }

        public static float DistanceSquared(in PointF p1, in PointF p2)
        {
            float dx = p1.X - p2.X;
            float dy = p1.Y - p2.Y;
            return dx * dx + dy * dy;
